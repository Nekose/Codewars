import unittest
from src import elemental_words
...
class MyTestCase(unittest.TestCase):
    test1 = [['Sulfur (S)', 'Nitrogen (N)', 'Actinium (Ac)', 'Potassium (K)'],
             ['Sulfur (S)', 'Sodium (Na)', 'Carbon (C)', 'Potassium (K)'],
             ['Tin (Sn)', 'Actinium (Ac)', 'Potassium (K)']]
    def test_match_keys(self):
        self.assertEqual(elemental_words.match_keys("snack",1,ELEMENTS), ["S"])
        self.assertEqual(elemental_words.match_keys("snack", 2, ELEMENTS), ["Sn"])
    def test_elemental_words(self):
        self.assertEqual(elemental_words.elemental_forms("snack"),None)
